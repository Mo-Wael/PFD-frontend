  ...
  return (
    <div className="flex flex-1 gap-2 items-start sm:items-center justify-between bg-white p-6 rounded-xl hover:shadow-lg transition-shadow w-fit">
      ...
      <div className="space-y-2 flex-1">
        ...
        <h1
          className={`text-2xl sm:text-4xl font-bold ${cardType === "income" ? "text-black" : "text-red-600"
            }`}
        >
          {formatCurrency(amount)}
        </h1>
        <p
          className={`text-xs sm:text-md ${cardType === "income" ? "text-green-500" : "text-red-500"
            }`}
        >
          {change}
        </p>
      ...
      <div
        className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 
          ${cardType === "income" ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
      >
        {icon}
      </div>
    </div>
